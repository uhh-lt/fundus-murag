...
class CollectionLexicalSearchQuery(LexicalSearchQueryBase):
    search_in_collection_name: bool = Field(
        default=True, description="Search in the name of the collection."
    )
    search_in_title: bool = Field(
        default=True, description="Search in the title of the collection."
    )
    search_in_description: bool = Field(
        default=True, description="Search in the description of the collection."
    )
    search_in_german_title: bool = Field(
        default=True, description="Search in the German title of the collection."
    )
...
class SimilaritySearchQuery(BaseModel):
    ...
    confidence_threshold: float = Field(
        default=0.0, description="The minimum confidence threshold."
    )
    collection_names: list[str] | None = Field(
        None,
