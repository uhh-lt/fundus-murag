...
def load_fundus_records_df(fundus_records_df_file: str | Path, dev_mode: bool = True) -> pd.DataFrame:
    fundus_records_df_file = Path(fundus_records_df_file)
    if not fundus_records_df_file.exists():
        raise FileNotFoundError(f"Cannot find FUNDus Records at {fundus_records_df_file.absolute()}")
    logger.info(f"Loading FUNDus Records from {fundus_records_df_file} ...")
    records_df = pd.read_parquet(str(fundus_records_df_file))
    logger.info(f"Loaded FUNDus Records with {len(records_df)} records from {fundus_records_df_file}!")
...
def load_fundus_record_embeddings_df(
    ...
    logger.info(f"Loading FUNDus Record Embeddings from {fundus_record_embeddings_df_file} ...")
    ...
    if dev_mode:
        if records_df is None:
            raise ValueError("Cannot load record embeddings in dev mode without loading the records DataFrame first!")
        logger.info("Using DataFrame in dev mode!")
        record_embeddings_df = record_embeddings_df[record_embeddings_df["fundus_id"].isin(records_df["fundus_id"])]
...
def load_fundus_collections_df(
    ...
    if not fundus_collections_df_file.exists():
        raise FileNotFoundError(f"Cannot find FUNDus Collections DataFrame at {fundus_collections_df_file.absolute()}")
    ...
    logger.info(f"Loaded FUNDus Collections with {len(collections_df)} records from {fundus_collections_df_file}!")
    ...
    collections_df["contacts"] = collections_df["contacts"].apply(replace_none_in_list_of_dicts)
    collections_df["fields"] = collections_df["fields"].apply(replace_none_in_list_of_dicts)
...
def load_fundus_collection_embeddings_df(
    ...
    logger.info(f"Loading FUNDus Collection Embeddings from {fundus_collections_embeddings_df_file} ...")
